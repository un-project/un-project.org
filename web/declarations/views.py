...
from declarations.forms import (ResolutionCreationForm, DeclarationCreationForm,
                            DeclarationEditForm, ReportForm)
from declarations.signals import (added_declaration_for_declaration,
...
class ResolutionDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        edit_mode = (
            ...
            resolution.user == self.request.user)
        ...
        return super(ResolutionDetailView, self).get_context_data(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if partial:
            ...
            try:
                serialized = resolution.partial_serialize(int(partial), self.request.user)
            except (StopIteration, ValueError):
                raise Http404

            return render(request, 'declarations/tree.html', {
                'declarations': serialized['declarations'],
                'serialized': serialized,
                'level': int(level)
            })

        return super(ResolutionDetailView, self).get(request, *args, **kwargs)
...
class ResolutionJsonView(DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        return HttpResponse(json.dumps({
            "nodes": self.build_tree(resolution, self.request.user),
        ...
        children = [{
            ...
            "speaker": {
                ...
                "absolute_url": reverse("auth_profile",
                                        args=[declaration.speaker.slug])
            },
            "sources": declaration.sources,
            "declaration_type": declaration.declaration_class(),
            "children": (self.get_declarations(resolution, user, parent=declaration)
                         if declaration.published_children().exists() else [])
...
class SearchView(HomeView):
    ...
    method_mapping = {'resolutions': "get_resolutions",
...
class FeaturedJSONView(HomeView):
    def render_to_response(self, context, **response_kwargs):
        resolutions = [resolution.get_overview_bundle()
...
class StatsView(HomeView):
    ...
    def get_active_speakers(self):
        return Speaker.objects.annotate(
            declaration_count=Sum("speaker_declarations"),
        ).filter(
            declaration_count__gt=0,
            **self.build_time_filters(date_field="speaker_declarations__date_creation")
    ...
    def get_disgraced_speakers(self):
        return Speaker.objects.annotate(
            report_count=Sum("speaker_declarations__reports"),
        ).filter(
            report_count__gt=0,
            **self.build_time_filters(date_field="speaker_declarations__date_creation")
    ...
    def get_supported_declarations(self):
        return Declaration.objects.annotate(
    ...
    def get_crowded_resolutions(self):
        return Resolution.objects.annotate(
            declaration_count=Sum("declarations"),
        ).filter(
            language=normalize_language_code(get_language()),
            declaration_count__gt=0,
            **self.build_time_filters(date_field="date_creation")
...
class ControversialResolutionsView(HomeView):
    ...
    def get_resolutions(self, paginate=True):
        ...
        resolutions = (Resolution
                       .objects
                       .annotate(num_children=Count('declarations'))
...
class ResolutionCreationView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        response = super(ResolutionCreationView, self).form_valid(form)
...
class ResolutionUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super(ResolutionUpdateView, self).form_valid(form)
        form.instance.update_sibling_counts()
...
class RandomResolutionView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        resolution = Resolution.objects.annotate(
...
class DeclarationEditView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super(DeclarationEditView, self).form_valid(form)
        form.instance.resolution.update_sibling_counts()
        return response

    def get_context_data(self, **kwargs):
        return super(DeclarationEditView, self).get_context_data(**kwargs)
...
class DeclarationCreationView(NextURLMixin, LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        return super(DeclarationCreationView, self).get_context_data(
    ...
    def form_valid(self, form):
        ...
        if form.instance.parent:
            added_declaration_for_declaration.send(sender=self,
                                           declaration=form.instance)
        else:
            added_declaration_for_resolution.send(sender=self,
                                              declaration=form.instance)
...
class DeclarationDeleteView(LoginRequiredMixin, View):
    def get_declaration(self):
        ...
        return get_object_or_404(declarations,
                                 pk=self.kwargs['pk'])
