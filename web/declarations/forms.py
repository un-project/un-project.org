...
class DeclarationCreationForm(FormRenderer, forms.ModelForm):
    ...
    def clean(self):
        ...
        if not form_data['related_resolution'] and not form_data['text']:
            raise ValidationError(_('You should write a declaration or link a resolution.'))
        return form_data
...
class DeclarationEditForm(DeclarationCreationForm):
    class Meta:
        model = Declaration
        fields = ['declaration_type', 'text', 'sources', 'parent',
                  'related_resolution', 'is_approved']
        widgets = {
            'declaration_type': forms.RadioSelect,
            'related_resolution': forms.TextInput,
        }
    ...
    def __init__(self, *args, **kwargs):
        super(DeclarationEditForm, self).__init__(*args, **kwargs)
        queryset = (
            self.instance.resolution.declarations
