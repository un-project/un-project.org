...
# Drop and recreate the unified search index so that resolution rows include
# full_text in both the content column (for headline snippets) and in the
# search_vector at weight 'D' (lower than title/category but still ranked).
#
# Also relaxes the WHERE filter to include SC resolutions that have full_text
# but no title.
...
FROM (
    ...
    SELECT
        ...
        coalesce(r.title, '') || ' ' || coalesce(r.full_text, '') AS content,
        (
